...
import numpy as np
import demoji
from transformers import RobertaConfig, RobertaTokenizer, RobertaModel, BertModel, BertForSequenceClassification, AdamW, BertConfig, BertTokenizer, get_linear_schedule_with_warmup
...
config = RobertaConfig.from_pretrained(roberta_model_name, output_hidden_states=True)
roberta = RobertaModel.from_pretrained(roberta_model_name, config=config)
...
for fold in range(5):
    ...
    model = BiGRU_Model(roberta, hidden_size, dropout)
    ...
    best_model = BiGRU_Model(roberta, hidden_size, dropout).to(device)
